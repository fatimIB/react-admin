...
const WithdrawTable = ({ token }) => {
  ...
  const fetchWithdraws = async () => {
    try {
      ...
      const response = await axios.get("http://127.0.0.1:8000/api/withdraws/all", {
        headers: {
    ...
    try {
      ...
      setWithdraws((prevWithdraws) =>
        prevWithdraws.map((withdraw) =>
          withdraw.id === selectedWithdraw.id
            ? { ...withdraw, status: response.data.status }
            : withdraw
        )
      );
      handleCloseModal();
  ...
  const getStatusStyle = (status) => ({
    backgroundColor: status === "pending" ? "#ffc107" : status === "paid" ? "#30a947" : "#e41125",
    color: "white",
    padding: "5px 10px",
    borderRadius: "5px",
          ...
          <TableBody>
            {withdraws.map((withdraw) => (
              <TableRow key={withdraw.id}>
                ...
                <TableCell align="center">{withdraw.method}</TableCell>
                <TableCell align="center">
                  <span style={getStatusStyle(withdraw.status)}>
                    {withdraw.status}
                  </span>
                </TableCell>
                <TableCell align="center">
